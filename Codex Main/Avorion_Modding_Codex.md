...
> [!NOTE]
> **A tried-and-failed fix attempt, confirmed by the log, that narrows the search space further:** switching `include("callable")` to `include("data/scripts/lib/callable.lua")` — matching the full-path convention this Codex recommends for `addScriptOnce()` — does **not** work, and the engine's own error message proves exactly why. The resulting `include error` listed every candidate it tried, and every one ended in `callable.lua.lua`: `.\data/scripts/lib/callable.lua.lua`, `<mod root>\.\data/scripts/lib/callable.lua.lua`, for every currently-loaded mod folder. `include()` unconditionally appends `.lua` to whatever string it's given — it does not check whether the argument already ends in `.lua` first. This is a **confirmed, mechanical fact**, independent of the still-open question above: **`include()`'s full-path convention takes the module name *without* the `.lua` extension** (`include("data/scripts/lib/callable")`), unlike `addScriptOnce()`/`addScript()`, which need it (`entity:addScript("data/scripts/entity/cosmicbuff.lua")`). Conflating the two conventions is an easy mistake — this Codex made it once, in a live fix, before catching it from the resulting log. The extension-less full path is what's currently live as of this writing; whether it resolves the *original* dynamic-attach failure this entry is about is not yet confirmed by a clean in-game retest — treat that as still open, not solved by this correction.
